...
// File @openzeppelin/contracts-upgradeable/token/ERC1155/ERC1155Upgradeable.sol@v5.0.2
...
// File @openzeppelin/contracts-upgradeable/token/ERC1155/extensions/ERC1155URIStorageUpgradeable.sol@v5.0.2
...
// File contracts/libraries/Payment.sol
...
// File contracts/season-pass/interfaces/ISeasonPassEvents.sol

// Original license: SPDX_License_Identifier: MIT
pragma solidity 0.8.20;

interface ISeasonPassEvents {
    event SeasonPassPurchased(
        uint256 indexed id,
...
// File contracts/season-pass/interfaces/ISeasonPass.sol

// Original license: SPDX_License_Identifier: MIT
pragma solidity 0.8.20;


interface ISeasonPass is ISeasonPassEvents, IERC1155 {
    ...
    error InvalidAmount();
    error SeasonPassTransferNotAllowed();
    error MaxSeasonPassCountExceeded();
    error SignatureExpired();

    function ospTreasuryAddress() external view returns (address ospTreasuryAddress);

    function setBaseURI(string calldata baseURI) external;

    function setURI(uint256 seasonId, string calldata baseURI) external;

    function purchase(PurchaseParams calldata params) external payable;

    function withdraw(address currency, address to) external;

    function clamReward(address currency, address to) external;

    function reward(address user, address currency) external view returns (RewardStruct memory);
}
...
library Constants {
    ...
    bytes32 constant CASHIER_ROLE = keccak256('CASHIER_ROLE');
    bytes32 constant SERVER_SIGNER_ROLE = keccak256('SERVER_SIGNER_ROLE');
    uint256 public constant OSP_REVENUE_SPLIT = 60;
    uint256 public constant INVITER_REVENUE_SPLIT = 20;
}


// File contracts/season-pass/SeasonPass.sol
...
contract SeasonPass is AccessControlUpgradeable, ERC1155URIStorageUpgradeable, ISeasonPass {
    using ECDSA for bytes32;
    using MessageHashUtils for bytes32;

    address public immutable ospTreasuryAddress;
    mapping(address => mapping(address => RewardStruct)) internal _rewards;
    mapping(address => uint256) public totalRewards;

    constructor(address _ospTreasuryAddress) {
        ospTreasuryAddress = _ospTreasuryAddress;
    }
    ...
    function setBaseURI(string calldata baseURI) public onlyRole(Constants.ADMIN_ROLE) {
        _setBaseURI(baseURI);
    }
    ...
    function setURI(
        ...
        _setURI(seasonId, baseURI);
    }

    function purchase(PurchaseParams calldata params) public payable {
    ...
    function reward(address user, address currency) public view returns (RewardStruct memory) {
        return _rewards[user][currency];
    }

    function withdraw(address currency, address to) public onlyRole(Constants.CASHIER_ROLE) {
        uint256 value = (
            address(0) == currency
        ...
        currency == address(0)
            ? Payment.payNative(to, value)
            : Payment.payERC20(currency, to, value);
    }

    function clamReward(address currency, address to) public {
